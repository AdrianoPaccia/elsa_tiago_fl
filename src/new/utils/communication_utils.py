...
def log_communication(
    federated_round: int, sender: int, receiver: int, data: list, log_location: str
):
    """
    ...
    if int(sender) < -1 or int(receiver) < -1:
        raise ValueError(
            "The ID of the receiver or sender is smaller than -1 and thus invalid."
        )
    ...
    _save_row_to_csv(
        [federated_round, sender, receiver, amount_of_bytes], path=log_location
    )
...
def _save_row_to_csv(row: list, path: str):
    ...
    if not os.path.isfile(path):
        with open(path, "a") as f:
            f.write(
                ",".join(["federated_round", "sender", "receiver", "amount_of_bytes"])
            )
            f.write("\n")
    if path is not None:
        with open(path, "a") as f:
